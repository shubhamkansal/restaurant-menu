...
def restaurant():
    ...
    if 'username' not in login_session:
        return render_template('restaurants.html', restaurants=List)
    return render_template(
        'myrestaurants.html', restaurants=List)
...
def newMenuItem(restaurant_id):
    ...
    if login_session['user_id'] != rest.user_id:
        return "<script>function myFunction() {alert('You are not authorized');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
        menu = MenuItem(name=request.form['name'], description=request.form['description'], price=request.form['price'], course=request.form['course'], restaurant_id=restaurant_id, user_id=rest.user_id)
        session.add(menu)
        session.commit()
...
def restaurantMenu(restaurant_id):
    rest = session.query(Restaurant).filter_by(id=restaurant_id).one()
    owner = getUserInfo(rest.user_id)
    menu = session.query(MenuItem).filter_by(restaurant_id=restaurant_id).all()
    if 'username' not in login_session or owner.id != login_session['user_id']:
        return render_template(
        'menu.html', restaurant=rest, menu=menu, restaurant_id=rest.id)
    return render_template(
        'mymenu.html', restaurant=rest, menu=menu, restaurant_id=rest.id)
...
def editRestaurant(restaurant_id):
    if 'username' not in login_session:
        return redirect('/login')
    rest = session.query(Restaurant).filter_by(id=restaurant_id).one()
    if rest.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not authorized');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
        if request.form['name']:
        ...
        return render_template(
            'editrestaurant.html', restaurant_id=restaurant_id, item=rest)
